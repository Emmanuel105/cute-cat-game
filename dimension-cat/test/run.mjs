...
// people: the wardrobe spreads clothes, children are children, and someone waves at the cat
{
  ...
  { const q = people.find((n) => n.constructor.name === 'Wanderer'); const there = pos().clone();
    ...
    pos().copy(there); }
  // two neighbours chat: they face each other, take turns, and the speaker's hands are up
  { const tk = game.npcs.find((n) => n.constructor.name === 'Talkers'); const [a, b] = tk.rigs; const face = Math.abs(((a.group.rotation.y - b.group.rotation.y + Math.PI) % (2 * Math.PI) + 2 * Math.PI) % (2 * Math.PI)); 
    let swaps = 0, last = tk.speaker; for (let i = 0; i < 420; i++) { game.loop(); if (tk.speaker !== last) { swaps++; last = tk.speaker; } } const talker = tk.rigs[tk.speaker];
    check(tk && face < 0.05 && swaps >= 1 && talker.arms[0].sh.rotation.x < -0.7, `two neighbours chat, facing each other, taking turns (${swaps} turns in 7 s, arm ${talker.arms[0].sh.rotation.x.toFixed(2)})`); }
  { const pt = game.npcs.find((n) => n.constructor.name === 'Painter'); pt.progress = 45; frames(2); check(pt && pt.easel.parent === game.world && pt.easel.userData.daubs.filter((d) => d.visible).length === 2 && pt.rig.arms[0].sh.rotation.x < -0.5, 'a painter at her easel by the pond, two daubs in'); }
...
for (const [idx, entry] of [[1, 'from-prev'], [2, 'from-prev'], [3, 'from-prev'], [2, 'from-next'], [1, 'from-next'], [0, 'from-next'], [4, 'from-hub'], [0, 'from-beach'], [5, 'from-hub'], [0, 'from-snow'], [6, 'from-hub'], [0, 'from-forest']]) {
  ...
  check(game.npcs.every((n) => { let o = n.rig.group; while (o.parent) o = o.parent; return o === game.scene; }), `world ${idx}: every NPC is in the scene graph`);
  ...
  check(game.friendTotal >= [12, 6, 4, 14, 8, 4, 2][idx], `world ${idx}: ${game.friendTotal} to meet`);
  while (game.transitioning) { await sleep(60); frames(4); }   // the portal fade ends on a real timer; interact() waits for it
  ...
  if (idx === 6) { const ch = game.npcs.find((n) => n.constructor.name === 'Chopper'); frames(600); check(ch && ch.chops >= 2 && ch.rig.hands.length === 2, `Whisper Woods: the woodcutter has split ${ch.chops} logs in 10 s`); }
  if (idx === 1) { const rd = game.npcs.find((n) => n.constructor.name === 'RingDance'); let walked = 0, px = rd.dancers[0].circle.x, pz = rd.dancers[0].circle.z; for (let i = 0; i < 60; i++) { game.loop(); const d = rd.dancers[0]; walked += Math.hypot(d.circle.x - px, d.circle.z - pz); px = d.circle.x; pz = d.circle.z; } check(walked > 0.8 && rd.dancers.every((d) => Math.abs(Math.hypot(d.circle.x - rd.cx, d.circle.z - rd.cz) - rd.r) < 0.01), `Candy Land: the gingerbread men dance round the lollipop (${walked.toFixed(1)} m walked in 1 s, all on the ring)`); }
  if (idx === 4) { const kf = game.npcs.find((n) => n.constructor.name === 'KiteFlyer'); const k0 = kf.kite.position.clone(); frames(90); check(kf && kf.kite.parent === game.world && kf.kite.position.y > kf.rig.group.position.y + 4.5 && kf.kite.position.distanceTo(k0) > 0.3 && kf.line.scale.z > 6, `Sunny Shore: a kite flies ${(kf.kite.position.y - kf.rig.group.position.y).toFixed(1)} m up on a ${kf.line.scale.z.toFixed(1)} m line`); }
  if (idx === 4) { const sb = game.npcs.filter((n) => n.constructor.name === 'Sunbather'), kn = game.npcs.find((n) => n.constructor.name === 'Kneeler'); check(sb.length === 3 && sb.every((n) => Math.abs(n.rig.group.rotation.x + Math.PI / 2) < 0.01) && kn && kn.rig.legs[0].knee.rotation.x > 1.5, 'Sunny Shore: three sunbathers lie on their towels and a child kneels at the sandcastle'); }
  ...
  if (idx === 5) { const sd = game.npcs.find((n) => n.constructor.name === 'Sledder'); const seen = new Set(); const yTop = game.physics.ground0(...sd.top), yBot = game.physics.ground0(...sd.bottom); for (let i = 0; i < 1500; i++) { game.loop(); seen.add(sd.state); } check(sd && seen.has('slide') && seen.has('climb') && sd.runs >= 1 && yTop - yBot > 3 && Number.isFinite(sd.rig.group.position.y), `Frosty Peak: the sledder has been down ${sd.runs} times and back up (a ${(yTop - yBot).toFixed(1)} m drop)`); }
