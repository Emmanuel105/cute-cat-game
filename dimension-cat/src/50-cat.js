...
class Cat {
  constructor() {
    ...
    this.groom = 0; this.groomT = 8; this.groomSide = 1;   // fixed, not rnd: the cat is built once at startup, and a draw here would shift every later call on the shared global rnd()
  }
  ...
  update(dt, t) {
    ...
    // grooming: sat still a while, every so often a lick at the flank — a duck of the head, turned aside, with a quick bobbing tongue
    this.groomT -= dt;
    if (!moving && S > 0.7 && this.groom <= 0 && this.groomT <= 0) { this.groom = 1; this.groomSide = rnd.chance(0.5) ? 1 : -1; this.groomT = rnd.range(9, 18); }
    if (this.groom > 0) this.groom = moving ? 0 : max(0, this.groom - dt / 2.2); else this.groom = 0;
    const G = sin(min(1, this.groom) * PI) * (this.groom > 0 ? 1 : 0);
    const headYaw = lerp(this.lookYaw, this.groomSide * 0.85, G), headPitch = lerp(this.lookPitch - L * 0.12 + S * 0.22 - ST * 0.5 + (moving ? sin(this.walkPhase * 2) * 0.02 : 0), 0.8 + sin(t * 13) * 0.08, G);
    P.head.rotation.set(headPitch, headYaw, sin(t * 1.1) * 0.015 * (1 - G));
