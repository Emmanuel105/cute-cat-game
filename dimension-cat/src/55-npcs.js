...
class RoboDog {
  constructor(game, path) {
    ...
    this.friend = false; this.friendId = game.namedFriend('robodog'); this.cryT = rnd.range(9, 16);
    ...
    // corner it, or just walk up — either way, a toy mouse buys its loyalty for good
    game.addInteractable({ obj: this.rig.group, radius: 2.6, label: () => (this.friend ? 'Good boy' : 'Offer a toy mouse'), onUse: () => {
      if (this.friend) { game.hearts(this.x, 0.7, this.z, 2); SFX.beep(); return; }
      this.friend = true; this.mode = 'patrol'; this.chaseT = 0; this.barkT = 0;
      game.befriend(this.friendId); SFX.beep(); SFX.woof();
      game.hearts(this.x, 0.7, this.z, 3); game.toast('🤖🐕 *drops the chase, sniffs the toy mouse* WOOF-BEEP! Friend detected!');
    } });
  }
  update(dt) {
    ...
    let tx, tz, speed;
    if (this.mode === 'patrol') {
      if (!this.friend && d2 < 49 && this.game.cat.onGround) { this.mode = 'chase'; this.chaseT = 7; this.barkT = 0; this.game.toast('🤖🐕 WOOF-BEEP! Intruder detected!'); }
    ...
    if (this.friend) { this.cryT -= dt; if (this.cryT <= 0) { this.cryT = rnd.range(10, 18); if (dist2(this.x, this.z, cat.x, cat.z) < 100) { SFX.woof(); this.game.toast('🤖🐕 *happy beeping*', 1800); } } }
  }
}
