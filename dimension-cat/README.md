...
Controllers give them something to do. `Wanderer` strolls around a home spot and takes an `avoid(x, z)` (the Neighborhood's strollers stay off the asphalt); it turns the head toward the cat, gives a wave the first time the cat comes up (gentlemen tip their hat instead) and, through `greet()`, puts a line on the HUD from the world's `GREETINGS` — children have their own. `Sitter` parks a person on a bench, hips at seat height. `Playmates` runs two children playing tag: "it" chases, the other weaves, a tag swaps them with a hop and a three-second head start. `DogWalker` is a stroller with a dog on a lead — the dog is a `Follower` whose `target` is the walker rather than the cat, and the lead is a cylinder stretched from hand to collar every frame (kept out of the ink pass with `noInk()`).
