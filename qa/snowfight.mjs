// Frosty Peak: the two children having a snowball fight, six frames over eight seconds, then the cat
// walks up and takes one on the nose.
//   node qa/snowfight.mjs <outdir>
...
const out = path.join(here, process.argv[2] || 'snowfight'); fs.mkdirSync(out, { recursive: true });
...
await page.evaluate(() => window.DC.travel(5, 'from-hub')); await page.waitForTimeout(2400); await settle(page, 0.8);
...
// side on to the fight: the cat south of it looking north
await look(-3.5, 1, 0, 6, 0.14);
for (let i = 0; i < 6; i++) { await settle(page, i ? 1.3 : 0.6); await shot(page, `fight-${i}`); }
// now walk up to one of them and wait for the throw
await look(-4.5, 4.2, 0, 3.5, 0.1);
for (let i = 0; i < 4; i++) { await settle(page, 0.9); await shot(page, `cat-${i}`); }
const st = await page.evaluate(() => { const f = window.DC.npcs.find((n) => n.constructor.name === 'SnowballFight'); return { throws: f.throws, catHit: f.catHit, toast: document.getElementById('msg').textContent }; });
console.log('fight', JSON.stringify(st));
