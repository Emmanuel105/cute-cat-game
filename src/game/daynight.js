...
export function updateDayNight() {
  ...
  // Worlds with a locked time of day (Moon = always night, Candy = always day) don't advance.
  if (world.lockTime === null) state.dayTime = (state.dayTime + 0.000008) % 1;
  else state.dayTime = world.lockTime;
  ...
  if (dayTime < 0.2 || dayTime > 0.8) {
    skyColor = new THREE.Color(0x0a0a2e); ambientInt = 0.34; sunInt = 0;
    starsMat.opacity = Math.min(1, (dayTime < 0.2 ? 0.2 - dayTime : dayTime - 0.8) * 10);
    moon.intensity = 0.9; hemi.groundColor.setHex(0x111133);
    ...
    skyColor = new THREE.Color(world.daySky); ambientInt = 0.55; sunInt = 1.3;
